...
std::vector<double> mvc_xy(std::vector<double> xdata, std::vector<double> ydata, int start, int end)
{
	...
	std::for_each(&xdata[start], &xdata[end], [&](const double dx) { accumx += (dx - mx) * (dx - mx); });

	std::for_each(&ydata[start], &ydata[end], [&](const double dy) { accumy += (dy - my) * (dy - my); });
	...
	for (int i = 0; i < static_cast<int>(xdata.size()); ++i) { accumCovar += (xdata[i] - mx) * (ydata[i] - my); }
