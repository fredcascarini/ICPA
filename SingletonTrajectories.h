...
public:
	...
	std::vector<std::string>		ret_list() const									//returns list_of_bond_types
	{ return list_of_bond_types; }

	std::string						find_bond_type(int index)							//finds type of specified index in list_of_types
	{ return list_of_bond_types[index]; }


	int								find_bond_type_index(std::string type_to_indx);		//finds index of specified type in list_of_bond_types
...
private:
	...
	void							add_bond_type(std::string type);					//adds type to list_of_types

	std::map<std::string, double>	dict_of_lengths;									//dict of bond lengths
	std::vector<std::string>		list_of_bond_types;									//list of bond types
